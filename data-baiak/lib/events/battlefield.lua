BATTLEFIELD = {
	...
	messages = {
		...
		messageStart = "O portal para o evento foi fechado e ser� iniciado com %d participantes! Boa sorte!",
		messageNoStart = "O evento n�o foi iniciado por falta de participantes!",
		messageFinish = "O evento acabou! O time %s foi o campe�o do evento!",
		messageWait = "O portal para o evento est� aberto, voc� tem %d minuto(s) para entrar!",
	...
	-- 0 n�o iniciou, 1 iniciou, 2 iniciou com ganhador azul, 3 iniciou com ganhador red, 4 iniciou sem ganhador
}
...
function BATTLEFIELD:startEvent()
	if Game.getStorageValue(BATTLEFIELD.storageEventStatus) >= 1 then
		for b in pairs(BATTLEFIELD["blue"].players) do
			...
			player:getPosition():sendMagicEffect(CONST_ME_TELEPORT)
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, BATTLEFIELD.messages.prefix .. "Em ".. BATTLEFIELD.timeRemoveWalls .." segundos os muros de madeira ser�o removidos!")
		end
