SAFEZONE = {
	...
	messages = {
		prefix = "[SAFEZONE] ",
		messageEntrou = "Voc� entrou no evento. Boa Sorte!",
		messageStart = "O evento ir� come�ar agora com %d participantes! Boa sorte!",
		messageNoStart = "O evento foi cancelado por falta de participantes!",
		messageTime = "O evento foi cancelado por limite de tempo!",
		messageOpen = "O evento foi aberto, voc� tem %d minutos para entrar no portal do evento que se encontra no templo!",
		messageWait = "O teleporte para o evento est� aberto, voc� tem %d minuto(s) para entrar!",
		messageFinish = "O evento foi finalizado e o ganhador foi o %s! Parab�ns!",
		messageWinner = "Voc� ganhou %d %s como premia��o e 300 Tournaments Coins.",
		messageLooser = "Voc� perdeu suas vidas e foi eliminado do Evento.",
...
function SAFEZONE:removePlayer(playerId)
	...
	--Spdlog.info("[SAFEZONE EVENT] - Valor i: "..i.. " valor v: "..v)
		if(v == playerId) then
			...
			--Spdlog.info("[SAFEZONE EVENT] - User exclu�do com sucesso. ")
			SAFEZONE.players[v] = nil
			table.remove(SAFEZONE.players,i)
		else
			--Spdlog.error("[SAFEZONE EVENT] - N�o exclu�do. ")
		end
	end
end	
--[[
		...
		for _, a in ipairs(SAFEZONE.players) do
			local player = Player(a)
				...
				if itemType:getId() ~= 0 then
					...
					db.query(string.format("INSERT INTO `store_history`(`account_id`, `mode`, `description`, `coin_type`, `coin_amount`, `time`) VALUES (%s, %s, %s, %s, %s, %s)", player:getAccountId(), "0", db.escapeString("[SafeZone Event] - Winner"), "1", "300", os.time()))
					Game.setStorageValue(SAFEZONE.started, 0)				
				...
				if tile then
					...
					if not item1 and not item2 and not item3 and not item4 then
						if player:getStorageValue(SAFEZONE.storage) > 1 then
							...
							player:sendTextMessage(MESSAGE_EVENT_ADVANCE, SAFEZONE.messages.prefix .. "Voc� perdeu uma vida, cuidado na pr�xima SafeZone!")							
						...
						player:sendTextMessage(MESSAGE_EVENT_ADVANCE, SAFEZONE.messages.prefix .. "Voc� Acertou a SafeZone !!")
					end
				end
			end
		end
