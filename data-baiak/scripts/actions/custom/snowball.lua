...
function snowball.onUse(player, item, fromPosition, itemEx, toPosition, isHotkey)
    if player:getStorageValue(10109) > 0 and player:getStorageValue(10108) <= 30  then
        ...
        player:sendTextMessage(MESSAGE_TRADE, SNOWBALL.prefixo .. (SNOWBALL.mensagemComprou):format(SNOWBALL.muniQuant, SNOWBALL.muniPreco, player:getStorageValue(10108), player:getStorageValue(10109)))
