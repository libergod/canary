...
function heartDestructionDevourer.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if not player:canFightBoss("World Devourer") then
		...
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You access to World Devourer was released!")
		item:transform(23687)
	else
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You access to World Devourer is already released!")
	end
